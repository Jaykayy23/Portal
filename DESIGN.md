...
description: A dispatch ledger for delivery work — ruled, tabular, and quiet, with one warm accent reserved for money and action.
colors:
  waybill-amber: "#f5a524"
  waybill-amber-ink: "#241a02"
  waybill-amber-text: "#965a00"
  waybill-amber-display: "#b87200"
  waybill-amber-soft: "#fdf3e0"
...
components:
  button-primary:
    backgroundColor: "{colors.waybill-amber}"
    textColor: "{colors.waybill-amber-ink}"
  ...
  button-mini:
    ...
    textColor: "{colors.waybill-amber-text}"
    rounded: "{rounded.control}"
    padding: "0 14px"
    height: "100%"
  input:
  ...
  stat-tile:
    ...
    padding: "12px 14px"
  badge-assigned:
    backgroundColor: "{colors.waybill-amber-soft}"
    textColor: "{colors.waybill-amber-text}"
...
above tabular figures, generous white, and one warm accent that appears only
...
numbers can be loud: a 24px amber price figure is the loudest thing on any screen,
and it earns that by being the number somebody is about to commit to. Colour is
signal, never atmosphere. Four role hues tell you whose seat you are in before you
...
- One accent family (amber) at four contrast-driven values, plus three role hues.
- Light-only, deliberately. Every ratio in the palette is verified against white.

## Colors

A monochrome shadcn neutral base carrying a four-value amber accent and three
role hues. The palette is unusual in that its variants exist for contrast
arithmetic, not for taste — each amber was picked to clear a specific ratio on a
specific surface, and the reasons are written into `app/globals.css`.

### Primary

- **Waybill Amber** (`#f5a524`): the identity colour, and **filled surfaces only** —
  the primary button, the logo fallback tile, the route's origin dot, the checkbox
  accent. It cannot reach 4.5:1 on white, so it never carries text.
- **Waybill Amber Ink** (`#241a02`): text on an amber fill (8.4:1). The only thing
  that sits on Waybill Amber.
- **Waybill Amber Text** (`#965a00`): the deepened ochre that carries all small
  amber text — active tab, mini-button label, agreed-price cell, assigned badge,
  the rider's cash-to-collect note. Verified 5.6:1 on white, 5.1:1 on the soft tint.
  Also the focus ring colour.
- **Waybill Amber Display** (`#b87200`): reserved for the 24px/600 price figure and
  the "due" stat tile. At that size WCAG's large-text bar is 3:1, which this clears
  at 3.9:1, so the headline figure stays vivid.
- **Waybill Amber Soft** (`#fdf3e0`): tinted background for assigned badges, amber
  callout blocks, and the "to pay the rider" panel on the link page.
...
Two hues that exist purely to give a seat an identity:

- **Dispatch Blue** (`#3b5bdb`, 5.9:1 on white) with **Dispatch Blue Soft** (`#e8ecfb`):
  the ops seat, and "owed" figures in the ledger.
- **Ledger Violet** (`#5f3dc4`, 7.1:1 on white, 6.3:1 on its own tint) with
  **Ledger Violet Soft** (`#efeafc`): the finance seat. It exists because the other
  three hues are all warm or green and finance needed a fourth that could not be
  confused with any of them.

### Neutral

- **Ink** (`oklch(0.145 0 0)`): all primary text.
- **Muted Ink** (`#707070`): labels, sub-lines, placeholders, inactive tabs.
...
- **Ledger White** (`oklch(1 0 0)`): the shell, cards, and the sticky table header
...
- **Ground Grey** (`oklch(0.97 0 0)`): the page behind the shell, and every recessed
  ...
  cards. One token doing both jobs is what makes recession read as "inside".
...
**The Four Ambers Rule.** The accent exists at four values because contrast forced
it, not because a scale looked tidy. `#f5a524` fills. `#965a00` writes. `#b87200`
writes only at ≥24px/600. `#241a02` is the only ink that sits on the fill. Picking
the wrong one is a legibility bug, not a taste difference.
...
**The Seat Hue Rule.** Teal is merchant, blue is ops, violet is finance, amber is
admin. Status badges reuse the amber and teal soft pairs (assigned, delivered) and
are distinguished by context and weight, not hue — so a new status or a fifth seat
needs a collision check before it needs a colour.
...
- **Display** (600, 24px, 1.2): the computed price. The largest type in the product,
  and the only place Waybill Amber Display appears at full strength.
...
container. Inside, structure is three stacked bands — header (`20px 26px`), tab
strip (`0 26px`, horizontally scrollable), and body (`26px`) — separated by hairlines.
...
- **Primary:** Waybill Amber fill with Waybill Amber Ink text, `12px` padding,
...
- **Mini:** the inline button beside an input — Ground Grey fill, Waybill Amber Text
  label at 600/12px, border brightens to amber on hover.
...
- **Focus:** border switches to Waybill Amber Text, native outline removed, and the
  global focus ring takes over.
- **Disabled:** `opacity: 0.6`. Placeholder is Muted Ink.
- **Checkbox chip:** a bordered `8px` chip that swaps its border to amber and its
...
- **Heading:** 600/14px +0.2px, `16px` bottom margin, with an optional amber ordinal
...
- **Tab strip:** horizontally scrollable row under the header, `4px` gaps,
  `13px 16px` per tab, 13.5px/500. Inactive is Muted Ink; hover goes to Ink; active
  is Waybill Amber Text with a `2px` amber bottom border. Each tab is a real route
  link, so panes are never hidden — the `somoFade` animation just softens the change.
- **Touch:** `44px` minimum height under `(pointer: coarse)`.

### Badges and Tags

- **Status badge:** `999px` pill, `3px 9px`, 11px/600, `white-space: nowrap`.
  Requested is neutral grey, assigned is amber-soft, delivered is teal-soft,
...
11px/1.45 muted sub-line. Six tones (`due` amber-display, `owed` dispatch-blue,
...
Two `10px` dots — amber for origin, teal for destination — joined by a `3px` Rule Grey
track that fills with an `amber → teal` gradient as the distance resolves, scaled in
...
```css
outline: 3px solid color-mix(in srgb, var(--amber) 58%, transparent);
outline-offset: 2px;
```
...
- **Do** pick the amber by its job — `#f5a524` to fill, `#965a00` to write, `#b87200`
  only at ≥24px/600, `#241a02` only on an amber fill.
...
- **Do** keep new radii on the ladder (6/8/9/10/12/14/999).
...
- **Don't** use `#f5a524` as a text colour. It cannot reach 4.5:1 on white.
...
- **Don't** add a gradient. This system has exactly two, and both are structural:
  the route line's `amber → teal` fill encodes direction of travel, and the radial
