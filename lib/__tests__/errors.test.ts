...
const OUR_RULES = [
  ...
  'Order #4f2a1 has already been settled for that part. Void the earlier settlement first if it was wrong.',
  'Order #4f2a1 is prepaid — the merchant was paid for the goods directly, so there is nothing to settle.',
  'Order #4f2a1: only GHS 12 is still owed on that part, so GHS 30 cannot be settled against it.',
  'The rider has not remitted the cash on order #4f2a1 yet, so it cannot be paid out.',
